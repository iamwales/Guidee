...
async def publish(r: redis.Redis, task_id: str, event: dict) -> None:
    event["task_id"] = task_id
    await r.publish(f"{TASK_CHANNEL_PREFIX}{task_id}", json.dumps(event))
    await r.hset(f"{TASK_PREFIX}{task_id}", mapping={
        k: str(v) for k, v in event.items() if v is not None
    })


async def process_task(r: redis.Redis, payload: dict) -> None:
    ...
    try:
        ...
        await r.hset(f"{TASK_PREFIX}{task_id}", mapping={"status": "failed", "error": str(e)})
        await publish(r, task_id, {"type": "error", "status": "failed", "message": str(e)})
